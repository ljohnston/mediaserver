#
# Use cases:
# - Build dev env via terraform
#   - ... and configure via ansible
# - Configure dev env via ansible
# - Destory dev env
# - Configure prod
# - ssh forward(s)?
#
...
TUNNEL_COMMAND := \
	ssh_cmd=$$(terraform/tf.sh ./ -chdir=terraform output -raw bastion_ssh_forward_command |sed 's/-N -L/-f -N -L/') && \
	...
	while ! nc -z localhost $$(terraform/tf.sh ./ -chdir=terraform output -raw bastion_ssh_forward_port) >/dev/null 2>&1; do \
		sleep 0.1; \
	done;

.PHONY: dev-plan
dev-plan:
	@terraform/tf.sh ./ -chdir=terraform plan

.PHONY: dev-infra
dev-infra:
	@terraform/tf.sh ./ -chdir=terraform plan -detailed-exitcode -out=tfplan >/dev/null; \
	exit_code=$$?; \
	if [ "$$exit_code" = "2" ]; then \
...
.PHONY: dev-destroy
dev-destroy:
	@terraform/tf.sh ./ -chdir=terraform destroy
